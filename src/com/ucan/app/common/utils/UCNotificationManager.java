...
public class UCNotificationManager {
    ...
    public static UCNotificationManager mInstance;
    public static UCNotificationManager getInstance() {
        if(mInstance == null) {
            mInstance = new UCNotificationManager(UCAppManager.getContext());
        }
    ...
    private UCNotificationManager(Context context){
        mContext = context;
    }
    ...
    public final void showCustomNewMessageNotification(Context context , String pushContent ,String fromUserName ,String sessionId ,  int lastMsgType) {
        LogUtil.w(LogUtil.getLogUtilsTag(UCNotificationManager.class),
        ...
        boolean shake = UCPreferences.getSharedPreferences().getBoolean(UCPreferenceSettings.SETTINGS_NEW_MSG_SHAKE.getId(), true);
        boolean sound = UCPreferences.getSharedPreferences().getBoolean(UCPreferenceSettings.SETTINGS_NEW_MSG_SOUND.getId(), true);
        int defaults;
        if((sound && shake)) {
    ...
    private void cancel() {
        NotificationManager notificationManager = (NotificationManager) UCAppManager
    ...
    public final void showKickoffNotification(Context context , String kickofftext) {
        ...
        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
        PendingIntent pendingIntent = PendingIntent.getActivity(mContext, 35, intent, PendingIntent.FLAG_UPDATE_CURRENT);
        Notification notification = new Notification(R.drawable.uc_launcher, null, System.currentTimeMillis());
