package com.ucan.app.common.contacts;
...
public class ContactLogic {
    ...
    static {
        try {
            if(mDefaultBitmap == null) {
                mDefaultBitmap = VeryUtils.decodeStream(UCAppManager.getContext().getAssets().open("avatar/personal_center_default_avatar.png"), ResourceHelper.getDensity(null));
            }
        } catch (IOException e) {
        }
    }
    ...
    public static Bitmap getPhoto(String username) {
        ...
        try {
            ...
            if(username.startsWith("mobilePhoto://")) {
                ...
                bitmap =  VeryUtils.decodeStream(UCAppManager.getContext()
     ...
     * @return
     */
    public static ArrayList<UCContacts> converContacts(ArrayList<UCContacts> beas) {

        if(beas == null || beas.isEmpty()) {
            return null;
        }
        Collections.sort(beas, new Comparator<UCContacts>() {

            @Override
            public int compare(UCContacts lhs, UCContacts rhs) {
        ...
        for(int i = 0 ; i < beas.size() ; i ++ ) {
            UCContacts accountBean = beas.get(i);
            if (i < 5) {
     ...
     * @return
     */
    public static UCArrayLists<UCContacts> getPhoneContacts(Context ctx) {
        UCArrayLists<UCContacts> contacts = new UCArrayLists<UCContacts>();
        ...
        try {
            ...
            while (cursor.moveToNext()) {
                ...
                UCContacts data = new UCContacts();
                data.setId(indexId);
    ...
    public List<String[]> getSettingList() {
        ...
        Cursor cursor = null;
        try {
            cursor = UCAppManager.getContext().getContentResolver().query(ContactsContract.Settings.CONTENT_URI,
    ...
    public static UCArrayLists<UCContacts> getContractList(boolean showSim) throws Exception {
        // TODO 可能需要添加，是否显示无号码联系人选项，显示哪些账户联系人选项
       getInstance().getSettingList();
        Cursor cursor = null;
        UCArrayLists<UCContacts> cl = null;
        ...
        try {
            cursor = UCAppManager.getContext().getContentResolver().query(uri, projection_getContractList, where,
                    WhereParams, ContactsContract.Data.DISPLAY_NAME + ","
                            + ContactsContract.Data.RAW_CONTACT_ID);
            if (cursor != null) {
                cl = new UCArrayLists<UCContacts>();
                UCContacts cli = null;
...
//		PinyinHelper.release();
    ...
    public static void pyFormat(UCContacts contact) {
        try {
...
//							char c = name.charAt(i);
    ...
    public static Bitmap getChatroomPhoto(final String groupid) {
        ...
        if(groupid.toUpperCase().startsWith("G")) {
            return BitmapFactory.decodeResource(UCAppManager.getContext().getResources() , R.drawable.group_head);
        }
        return mDefaultBitmap;
    }
    ...
    private static List<BitmapUtil.InnerBitmapEntity> getBitmapEntitys(int count) {
        ...
        String value = UCPropertiesUtil.readData(UCAppManager.getContext(), String.valueOf(count), R.raw.nine_rect);
    ...
    public static void getMobileContactPhoto(List<UCContacts> list) {
        if(list == null || list.isEmpty()) {
            return ;
        }
        for(UCContacts contact : list) {
    ...
    public static void getMobileContactPhoto(UCContacts contact) {
        try {
    ...
    public static Bitmap getContactPhoto(UCContacts contact){
