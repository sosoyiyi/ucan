...
public class IMChattingHelper implements OnChatReceiveListener,
	...
	public void getPersonInfo() {
		...
		final ClientUser clientUser = UCAppManager.getClientUser();
		if (clientUser == null) {
			return;
		}
		LogUtil.d(TAG,
		...
		if (clientUser.getpVersion() < mServicePersonVersion) {
			SDKCoreHelper.getECChatManager().getPersonInfo(
						...
						public void onGetPersonInfoComplete(ECError e,
							...
							if (e.errorCode == SdkErrorCode.REQUEST_SUCCESS
								...
								try {
									UCPreferences
											.savePreference(
													UCPreferenceSettings.SETTINGS_REGIST_AUTO,
													newVersion, true);
	...
	private static void showNotification(ECMessage msg) {
		if (checkNeedNotification(msg.getSessionId())) {
			UCNotificationManager.getInstance().forceCancelNotification();
			String lastMsg = "";
			if (msg.getType() == ECMessage.Type.TXT) {
				lastMsg = ((ECTextMessageBody) msg.getBody()).getMessage();
			}
			UCContacts contact = ContactSqlManager.getContact(msg.getForm());
			if (contact == null) {
				return;
			}
			UCNotificationManager.getInstance()
					.showCustomNewMessageNotification(
							UCAppManager.getContext(), lastMsg,
	...
	public static boolean checkNeedNotification(String contactId) {
		String currentChattingContactId = UCPreferences
				.getSharedPreferences()
				.getString(
						UCPreferenceSettings.SETTING_CHATTING_CONTACTID.getId(),
						(String) UCPreferenceSettings.SETTING_CHATTING_CONTACTID
								.getDefaultValue());
		if (contactId == null) {
			return true;
		}
	...
	public void onReceiveOfflineMessageCompletion() {
		...
		// 无需要同步的消息
		UCAppManager.getContext().sendBroadcast(
