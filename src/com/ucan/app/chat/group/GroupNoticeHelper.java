 ...
 */
package com.ucan.app.chat.group;
...
public class GroupNoticeHelper {
	...
	public static CharSequence getNoticeContent(String content) {
		...
		if(content.indexOf("<admin>") != -1 && content.indexOf("</admin>") != -1) {
			...
			UCContacts contact = ContactSqlManager.getContact(contactId);
		...
		if(content.indexOf("<member>") != -1 && content.indexOf("</member>") != -1) {
			...
			UCContacts contact = ContactSqlManager.getContact(member);
    ...
    public static void insertNoticeMessage(ECGroupNoticeMessage instanceMsg ,OnPushGroupNoticeMessageListener l)  {
		...
		GroupNotice groupNotice = new GroupNotice(instanceMsg.getType().ordinal());
        String verifyMsg = "";
		...
		if(type == ECGroupNoticeMessage.ECGroupMessageType.JOIN) {// 直接加入
			...
			needSync = UCAppManager.getUserId().equals(joinMsg.getMember());
            ...
            if(UCAppManager.getUserId().equals(joinMsg.getMember())) {
            ...
            if(TextUtils.isEmpty(removeMsg.getNickName())) {
                UCContacts c = ContactSqlManager.getContact(removeMsg.getMember());
                if(c != null) {
            ...
            if(UCAppManager.getUserId().equals(removeMsg.getMember())) {
				needSync = true;
                ...
                UCAppManager.sendRemoveMemberBR();
			...
			needSync = UCAppManager.getUserId().equals(quitMsg.getMember());
        } else if (type == ECGroupNoticeMessage.ECGroupMessageType.REPLY_INVITE) {
            ...
            if(rInviteMsg.getConfirm() == 2) {
            	...
            	if(rInviteMsg.isDiscuss()){
            		...
            		verifyMsg = "讨论组管理员邀请 ["  + rInviteMsg.getNickName() + "]加入了讨论组";
            	}else {
			...
			UCContacts contact = ContactSqlManager.getContact(modifyGroupMsg.getMember());
			if(contact == null) {
